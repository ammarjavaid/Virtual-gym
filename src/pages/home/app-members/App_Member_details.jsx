...
import "./app_member_details.scss";
import {
  Client__details,
  Eye,
  Minus,
...
// import Message from "";
import { Button, DatePicker, Modal, Select, Space, Tag, message } from "antd";
...
const App_Member_details = () => {
  ...
  // console.log(formValues.exercises, selectedWorkout, "form values ss");

  const handleSingleWorkoutSubmit = async () => {
    setIsFormSubmitted(true);
    const validationErrors = validateForm();

    const checkExerciesExistsAndCompletedInSelectedWorkout = (id) => {
      const e = selectedWorkout?.exercise?.find((el) => {
        // console.log(el);
      ...
      // console.log(e, "123");
      return { status: e?.complete == "true", exercise: e };
    };
      ...
      try {
        const res = await ApiCall({
          params: {
            ...
            exercise: allExcercises
              .filter((e) => formValues.exercises?.includes(e?._id))
              .map((exercise) => {
                const searched =
                  checkExerciesExistsAndCompletedInSelectedWorkout(
                    exercise?._id
                  );
                // console.log(searched?.status, exercise?._id, "status ammar");
                if (searched?.status) {
                  return {
                    ...exercise,
                    submitted_sets: searched?.exercise?.submitted_sets,
                    submitted_notes: searched?.exercise?.submitted_notes,
                    submitted_time: searched?.exercise?.submitted_time,
                    complete: searched?.exercise?.complete,
                  };
                } else {
        ...
        if (res?.status == 200) {
          // console.log(res?.response?.workout, "res");
          setIsModalOpen(false);
          dispatch(setLoader(false));
          getClientPlan();
          handleOk();
          ...
          // setSelectedWorkout(null);
          setSelectedWorkout(res?.response?.workout);
  ...
  // console.log(allExcercises, "allExercise");
  ...
  return (
    <>
      <Topbar
        title="App member details"
        arrow={true}
        titleOne="App member"
        titleTwo="App member details"
        backLink="/app-members"
      />
        ...
        <ul>
          <li
            onClick={() => {
              ...
              navigate(`/app-members/member-details/${id}?tab=PersonalInfo`);
            }}
            className={
          ...
          <li
            onClick={() => {
              ...
              navigate(`/app-members/member-details/${id}?tab=Exercises`);
            }}
            className={
          ...
          <li
            onClick={() => {
              ...
              navigate(`/app-members/member-details/${id}?tab=Message`);
            }}
            className={activeTabs === "Message" ? "client-details-active" : ""}
          >
            Message
          </li>
        </ul>
...
export default App_Member_details;
